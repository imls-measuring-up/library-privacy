...
def open_html_file(name):
    open_html_file.data = open(name + '.html')  # open membership HTML page
...
def get_urls(org, num, tags, check, writer):
    for tag in tags[num].find_all('a'):
        try:
            if tag is not None:
                next_url = tag.get('href', None).encode("utf-8")
                if not next_url.startswith("http"):
                    continue
                next_name = tag.contents[0].encode("utf-8")
            ...
            print
            print "something is wrong with the HTML tag: {}".format(e)
            print


# create .CSV files of all the research library names and URLS extracted from DLF, ARL, and OCLC membership HTML files.
